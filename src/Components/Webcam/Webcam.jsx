    ...
    function Webcam() {
        ...
        React.useEffect(() => {
            if (totalScore>=5) {
                setWhoWon(userScore > computerScore ? 'You' : computerScore > userScore ? 'Computer':'Draw')
                console.log(whoWon);
                setIsGameStarted(false)
                setIsGamePaused(false)
        ...
        const whoWins = (userChoice) => {
            ...
            setLogs((oldLogs) => [...oldLogs, `Player choosed ${userChoice} & Computer choosed ${computerChoice}`])
        }
        ...
        return (
            <div>
                        ...
                        <div className="h-full flex justify-center items-center bg-black">
                            {
                                (isGameStarted === true && isGamePaused === false) ? (
                                    <Camera onGestureChange={setGestureText} isRunning={isGameStarted && !isGamePaused} />
                                ) : (isGamePaused === true) ? (
                ...
                <dialog id="my_modal_5" className="max-w-[450px] bg-white p-8 rounded-lg">
                    <div className="modal-box">
                        <h3 className="font-bold text-3xl">
                            {whoWon == "Computer" ? '😔Computer won better luck next time!!' : whoWon =="You"?'🥳Congratulations You Won!!':'Game is Draw'}
                        </h3>
                        <p className="mt-4 mb-8">Do you want to challange the Computer again?</p>
                        <div className="modal-action">
                            <form method="dialog" className='space-x-4'>
                                <button className="btn py-3 px-4 border-1 rounded border-gray-200 bg-blue-400" onClick={reset}>Close</button>
